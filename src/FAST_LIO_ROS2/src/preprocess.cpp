...
void Preprocess::velodyne_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  // Robust field detection for simulation
  ...
  if (has_intensity && has_time && has_ring) {
    pcl::fromROSMsg(*msg, pl_orig);
  } else {
    // Fallback for simulation LiDARs (e.g. Isaac Sim Hesai XT32)
    if (has_intensity) {
        pcl::PointCloud<pcl::PointXYZI> pl_temp;
        pcl::fromROSMsg(*msg, pl_temp);
        pl_orig.points.resize(pl_temp.size());
        for (size_t i = 0; i < pl_temp.size(); i++) {
            pl_orig.points[i].x = pl_temp.points[i].x;
            pl_orig.points[i].y = pl_temp.points[i].y;
            pl_orig.points[i].z = pl_temp.points[i].z;
            pl_orig.points[i].intensity = pl_temp.points[i].intensity;
            pl_orig.points[i].time = 0.0;
            pl_orig.points[i].ring = (uint16_t)(i % N_SCANS); // Mock ring
        }
    } else {
        pcl::PointCloud<pcl::PointXYZ> pl_temp;
        pcl::fromROSMsg(*msg, pl_temp);
        pl_orig.points.resize(pl_temp.size());
        for (size_t i = 0; i < pl_temp.size(); i++) {
            pl_orig.points[i].x = pl_temp.points[i].x;
            pl_orig.points[i].y = pl_temp.points[i].y;
            pl_orig.points[i].z = pl_temp.points[i].z;
            pl_orig.points[i].intensity = 1.0;
            pl_orig.points[i].time = 0.0;
            pl_orig.points[i].ring = (uint16_t)(i % N_SCANS); // Mock ring
        }
    }
