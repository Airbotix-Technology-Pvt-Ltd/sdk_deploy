...
> ---
>
> # ⚡ Airbotix Optimized Fork for Lite3
>
> This fork has been re-engineered by **Airbotix Technology Pvt Ltd** for high-fidelity **Isaac Sim** simulation and physical **Lite3** deployment.
>
> ### **🛠 What We Changed:**
> - **Dependency Stripping**: Formally removed the mandatory `livox_ros_driver2` dependency for a faster, hardware-agnostic build.
> - **Simulation Robustification**: 
>   - Synchronized IMU QoS with simulation hardware (`SensorDataQoS`).
>   - Implemented robust fallback logic in `preprocess.cpp` for missing LiDAR fields (`intensity`, `time`, `ring`) common in simulation sensors.
> - **Framework Synchronization**: Exposed the `child_frame_id` as a parameter for perfect TF alignment with `lidar_link` / `imu_link`.
> - **Config Hub**: Added [**`xt32.yaml`**](config/xt32.yaml) perfectly calibrated for **Hesai XT32** simulation in Isaac Sim.
>
> ### **🔨 How to Build:**
> ```bash
> cd Lite3_sdk_deploy
...
>
> ### **💾 How to Save the Map:**
> Trigger the specialized service while the node is active:
> ```bash
> ros2 service call /map_save std_srvs/srv/Trigger "{}"
> ```
> The map will be saved to the path specified in `xt32.yaml` (default: `./simulation.pcd`).
>
> ### **🗺️ How to Convert to 2D (Nav2):**
> 1. Install prerequisites: `sudo apt install pcl-tools`
> 2. Convert binary PCD to ASCII:
>    ```bash
>    pcl_convert_pcd_ascii_binary simulation.pcd simulation_ascii.pcd 0
>    ```
> 3. Run our custom projection script:
>    ```bash
>    python3 pcd_to_grid.py
>    ```
>    This generates `map.pgm` and `map.yaml` for the Nav2 stack.
>
> ---
